...
export class HyosanChatSettingsButton extends ShoelaceElement {
  ...
  private _handleClickSettings() {
    const dialog = this.shadowRoot?.querySelector('.settings-dialog') as SlDialog
  ...
  private _onFieldChange(event: CustomEvent<object>, field: keyof ChatSettings) {
